...
def get_breakpoint(location=None, expression=None, condition=None, number=None):
    ...
    if location is None and expression is None and number is None:
        return get_last_breakpoint()
    bps = gdb.breakpoints()
    print("input parameter number type={}".format(type(number).__name__))
    print("get_breakpoint number={} in total {}".format(number,len(bps)))
    if bps is None: raise gdb.GdbError('No breakpoints or watchpoints.')

    if number is not None:
        for bp in bps:
            print("bp.number type:{}".format(type(bp.number).__name__))
            print("input number type:{}".format(type(number).__name__))
            if bp.number == number:
                return bp
    else:
        for bp in bps:
            if (bp.location == location and bp.expression == expression and
                ...
                return bp
    print("not found breakpoint:{}".format(number))
    return None
...
def stop(callback, breakpoint=None, remove=False):
    ...
    if not remove:
        if isinstance(breakpoint, gdb.Breakpoint):
            ...
            traceback.print_stack()
            print("not remove breakpoint stop callback")
            gdb.events.stop.connect(callback)
    else:
        print("remove stop callback")
        if isinstance(breakpoint, gdb.Breakpoint):
            traceback.print_stack()
            print("remove_callback_to_breakpoint_num:{}".format(breakpoint.number))
            remove_callback_to_breakpoint_num(breakpoint.number, callback)
            print("remove breakpoint specific stop callback")
        else:
            traceback.print_stack()
            gdb.events.stop.disconnect(callback)


def tstop(callback, breakpoint):
    """Run callback once while gdb stops on breakpoints.
    If `breakpoint` is given, run it while specific breakpoint is hit.
    If `remove` is True, remove callback instead of adding it.
    """
    if isinstance(breakpoint, gdb.Breakpoint):
        traceback.print_stack()
        register_temp_callback_to_breakpoint_num(breakpoint.number, callback)
...
def register_callback_to_breakpoint_num(breakpoint_num, callback):
    print("register_callback_to_breakpoint_num:{}".format(breakpoint_num))
...
def remove_callback_to_breakpoint_num(breakpoint_num, callback):
    if callback in STOP_EVENT_REGISTER[breakpoint_num]:
        print("remove_callback_to_breakpoint_num:{}".format(breakpoint_num))
        STOP_EVENT_REGISTER[breakpoint_num].remove(callback)

def trigger_registered_callback(num):
    print("trigger_registered_callback:",num)
    if num in STOP_EVENT_REGISTER:
        for cb in STOP_EVENT_REGISTER[num]:
            cb()
    if num in TEMP_STOP_EVENT_REGISTER:
        for cb in TEMP_STOP_EVENT_REGISTER[num]:
            print("trigger_registered_callback once and remove")
            cb()
            remove_temp_callback_to_breakpoint_num(num,cb)


TEMP_STOP_EVENT_REGISTER=defaultdict(set)
def register_temp_callback_to_breakpoint_num(breakpoint_num, callback):
    print("temp_register_callback_to_breakpoint_num:{}".format(breakpoint_num))
    TEMP_STOP_EVENT_REGISTER[breakpoint_num].add(callback)

def remove_temp_callback_to_breakpoint_num(breakpoint_num, callback):
    if callback in STOP_EVENT_REGISTER[breakpoint_num]:
        print("temp_remove_callback_to_breakpoint_num:{}".format(breakpoint_num))
        TEMP_STOP_EVENT_REGISTER[breakpoint_num].remove(callback)
...
def stop_handler(event):
    ...
    if isinstance(event, gdb.BreakpointEvent):
        ...
        print("breakpoints num",len(event.breakpoints)) 
        for bp in event.breakpoints:
            trigger_registered_callback(bp.number)

print("gdb.events.stop.connect")
...
def eval_template(template, cmd):
    ...
    print 'command definition:',class_definition
    #print 'namespace2:',namespace
    result = namespace[classname]
    print(type(result))
    print result
    result.cmd = staticmethod(cmd)
    result._source = class_definition
    try:
        print 'frame1:',sys._getframe(1)
        #print 'globals:',sys._getframe(1).f_globals
        result.__module__ = sys._getframe(1).f_globals.get(
            '__name__', '__main__')
        print 'module:',result.__module__
...
def build_pprinter(to_string, display_hint=None, children=None):
    ...
    exec(class_definition, namespace) in globals(),locals()
    result = namespace[classname]
